...
## 新しいスタッフの追加（かんたん）
新メンバーが来たときは、基本 **コード編集なし** で増やせます。

**最短（コード不要）**
1. その人の「名前」シート（スロット式のタブ）をスプレッドシートに用意する。
2. URL を `?staff=<タブ名>&token=xxxx` の形で渡す（例：タブ名が `Liisa` なら `?staff=Liisa&token=xxxx`）。
   - `CONFIG.ALLOW_ANY_SHEET = true`（既定）なので、登録していない名前でもそのタブに書き込めます。
   - 画面の表示名はタブ名がそのまま出ます。

**任意（見た目を整える場合）**
`Code.gs` の `CONFIG.STAFF` に1行足すと、短いキーや表示名を設定できます。
```js
liisa: { name: 'Liisa', sheet: 'Liisa' },   // ?staff=liisa で Liisa シートへ
```
追加後は **デプロイを管理 → 新バージョン → デプロイ** で反映（URL は不変）。

> 厳格運用にしたい（登録した人だけ許可）場合は `CONFIG.ALLOW_ANY_SHEET = false` に。

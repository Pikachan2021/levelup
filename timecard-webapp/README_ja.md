...
スタッフがセルを直接いじらず、**自分専用 URL を開いて項目ボタンをタップするだけ**で、
スプレッドシート「Emeli T＆C」シートの **今日の日付の行・該当列に現在時刻が自動入力**される仕組みです。

- スマホ対応・ログイン不要・無料（Google Apps Script）
...
各自スマホで開き、**ホーム画面に追加**すればアプリのように使えます。

---

## 使い方（スタッフ向け）
1. 自分の URL を開く（ホーム画面のアイコンをタップ）。
2. 上に自分の名前と今日の日付が出ているか確認。
3. **「パッキング」または「かんぱにワーク」をタップ** → その瞬間の時刻が記録され、画面に「✓ …を記録しました」と表示。
